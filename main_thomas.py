...
for i, (train, valid, test, contfeats, binfeats) in enumerate(dataset.get_train_valid_test()):
    ...
    p_y_zt_dist = p_y_zt(dim_in=args.z_dim, nh=3, dim_h=args.h_dim, dim_out=1) 
    ...
    q_y_xt_dist = q_y_xt(dim_in=x_dim, nh=3, dim_h=args.h_dim, dim_out=1) 
    ...
    for epoch in range(n_epoch):
        ...
        if epoch % args.print_every == 0:
            print('Epoch %i' % epoch)
            y0, y1 = get_y0_y1(p_y_zt_dist, q_y_xt_dist, q_z_tyx_dist, torch.tensor(xalltr) ,
                               torch.tensor(talltr) )
